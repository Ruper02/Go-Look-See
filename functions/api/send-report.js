...
export async function onRequestPost({ request, env }) {
  ...
  const attachmentName = String(report.name || "informe-go-look-see.html").replace(/[^A-Za-z0-9._-]+/g, "-").slice(0, 150);
  try {
    ...
    if (hasWorkerEmailBinding) {
      const result = await env.EMAIL.send({
        ...message,
        attachments: [{ content: attachmentBuffer, filename: attachmentName, type: "text/html", disposition: "attachment" }],
      });
    ...
    const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(env.CLOUDFLARE_ACCOUNT_ID)}/email/sending/send`, {
      ...
      body: JSON.stringify({
        ...message,
        attachments: [{ content: arrayBufferToBase64(attachmentBuffer), filename: attachmentName, type: "text/html", disposition: "attachment" }],
      }),
    });
