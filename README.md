...
- Botón `Hacer foto` para abrir directamente la cámara del móvil, además de `Elegir imagen` para seleccionar un archivo existente.
...
Después, en Cloudflare Pages → Settings → Bindings, asigna la base D1 con el nombre `DB` y el bucket R2 con el nombre `PHOTOS`. Para el correo, verifica un dominio remitente en Cloudflare Email Service, crea un API Token con permiso de envío y añade en Pages → Settings → Variables and Secrets:
...
`wrangler.toml.example` sirve como referencia. No subas credenciales, tokens ni secretos al repositorio. La API REST de Email Service permite enviar desde una Function sin un binding de Worker.
