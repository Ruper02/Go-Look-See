 ...
 * Aplicación estática: no necesita servidor ni dependencias externas.
 *
 * Para personalizar la guía, edita el array CHECKLISTS de este archivo.
 */

const RECIPIENTS = [
  "acastillme@iberia.es",
...
function updateSaveIndicator(state = "saved") {
  ...
  element.innerHTML = state === "saving"
    ...
    : '<span class="save-dot"></span> Guardado localmente';
}
...
function renderDashboard() {
  ...
  const openMeasures = completed.reduce((sum, inspection) => sum + getFindings(inspection).filter((item) => ["improve", "critical"].includes(item.status)).length, 0);
...
async function handlePhotoFiles(files) {
  ...
  for (const file of files) {
    try {
      ...
      const photo = { id: uid("photo"), name: file.name, dataUrl, createdAt: new Date().toISOString() };
...
function removeCustomItem(itemId) {
  ...
  (item.photos || []).forEach((photoId) => deletePhoto(photoId));
  saveCurrentInspection();
  renderChecklist();
}
...
async function buildReportHtml(inspection) {
  ...
  const findingsHtml = findings.length
    ? findings.map((item) => {
      ...
      const meta = [item.owner ? `Responsable: ${escapeHtml(item.owner)}` : "", item.dueDate ? `Fecha objetivo: ${escapeHtml(formatDate(item.dueDate))}` : ""].filter(Boolean).join(" · ");
      return `
    ...
    : '<p class="report-general-notes">No se han registrado medidas abiertas en esta revisión.</p>';
...
async function downloadReport() {
  ...
  link.href = url;
  link.download = `go-look-see-${slugify(currentInspection.areaName || "revision")}-${currentInspection.date || todayISO()}.html`;
...
async function openEmail() {
  ...
  const findings = getFindings(currentInspection).filter((item) => ["improve", "critical"].includes(item.status));
  ...
  const lines = [
    ...
    ...findings.slice(0, 12).map((item, index) => `${index + 1}. [${statusInfo(item.status).short}] ${item.title} — ${item.action || item.note || "Pendiente de definir"}`),
    "",
    "Un saludo.",
  ];
...
function historyCardHtml(inspection) {
  ...
  const findings = getFindings(inspection).filter((item) => ["improve", "critical"].includes(item.status)).length;
  return `
...
async function deleteInspection(id) {
  const inspection = findInspection(id);
  if (!inspection) return;
  if (!window.confirm("¿Eliminar esta revisión y sus fotografías del dispositivo? Esta acción no se puede deshacer desde la aplicación.")) return;
  ...
  updateHeaderCount();
  renderDashboard();
  showToast("Revisión eliminada del dispositivo.");
}
