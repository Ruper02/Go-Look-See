...
// El backend gratuito guarda la fotografía comprimida como BLOB en D1.
// Se deja margen frente al límite de 2 MB por fila de D1.
const MAX_SHARED_PHOTO_BYTES = 1_400_000;
...
async function syncInspectionToServer(inspection) {
  ...
  try {
    ...
    if (error.status === 401) {
      sharedMode = false;
      sharedUser = null;
      setConnectionState("local", "Acceso requerido", "La sesión ha caducado. Introduce de nuevo la clave de la aplicación.");
      showLoginDialog();
    } else {
...
async function syncFromServer({ initial = false } = {}) {
  ...
  try {
    ...
    if (error.status === 401) {
      sharedMode = false;
      sharedUser = null;
      setConnectionState("local", "Acceso requerido", "Introduce la clave de la aplicación para activar el historial común.");
      if (initial || !$("#login-dialog")?.open) showLoginDialog();
    }
    return false;
  } finally {
...
async function handleLoginSubmit(event) {
  ...
  try {
    ...
    $("#login-form")?.reset();
    await initialiseSharedMode();
  } catch (error) {
    if (errorElement) errorElement.textContent = error.message || "No se ha podido iniciar sesión.";
  } finally {
    loginBusy = false;
        ...
        <div class="detail-grid">
          ...
          <div class="photo-area">
            ...
            <div class="photo-actions">
              <button class="photo-add photo-camera" type="button" data-action="take-photo" data-item-id="${item.id}"><span aria-hidden="true">◉</span> Hacer foto</button>
              <button class="photo-add" type="button" data-action="add-photo" data-item-id="${item.id}"><span aria-hidden="true">▧</span> Elegir imagen</button>
            </div>
          </div>
        </div>
      </div>
...
async function compressImage(file) {
  ...
  try {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      if (blob.size <= MAX_SHARED_PHOTO_BYTES) return blobToDataUrl(blob);
      scale *= 0.82;
    }
  } finally {
    if (bitmap.close) bitmap.close();
  }
  throw new Error("La fotografía es demasiado grande incluso después de comprimirla");
}
...
function waitForReportImage(image) {
  if (image.complete) {
    if (image.naturalWidth > 0) return Promise.resolve();
    return Promise.reject(new Error("No se ha podido cargar una fotografía del informe."));
  }
  return new Promise((resolve, reject) => {
    let timer;
    const cleanup = () => {
    ...
    const handleError = () => {
      cleanup();
      reject(new Error("No se ha podido cargar una fotografía del informe."));
    };
...
async function createPdfReportFile(body, filenameBase) {
  if (typeof window.html2pdf !== "function") {
    throw new Error("No se ha cargado el generador de PDF. Recarga la página e inténtalo de nuevo.");
  }

  const host = document.createElement("div");
  host.className = "report-pdf-host";
  ...
  host.innerHTML = `<style>${reportStandaloneCss()}</style>${body}`;
  document.body.appendChild(host);

  try {
    if (document.fonts?.ready) await document.fonts.ready;
    await Promise.all(Array.from(host.querySelectorAll("img")).map(waitForReportImage));
    ...
    const pdfBlob = await window.html2pdf()
      ...
      .from(sheet)
      .outputPdf("blob");
    return new File([pdfBlob], `${filenameBase}.pdf`, { type: "application/pdf" });
  } finally {
    host.remove();
  }
}
...
async function createReportFile() {
  ...
  const filenameBase = `go-look-see-${slugify(currentInspection.areaName || "revision")}-${currentInspection.date || todayISO()}`;
  return createPdfReportFile(body, filenameBase);
}
...
async function downloadReport() {
  try {
    ...
    showToast("PDF descargado. Puedes adjuntarlo al correo.");
  } catch (error) {
    showToast(error.message || "No se ha podido generar el PDF.");
  }
}
...
function reportStandaloneCss() {
  return `
    ...
    *{box-sizing:border-box}body{margin:0;padding:25px}@page{size:A4;margin:0}
...
function wireEvents() {
  ...
  $("#btn-login-local").addEventListener("click", () => {
    const dialog = $("#login-dialog");
    if (dialog?.open) dialog.close();
    setConnectionState("local", "Modo local", "La aplicación funciona en este dispositivo, pero aún no se ha activado el historial común.");
  });
  $("#photo-input").addEventListener("change", (event) => handlePhotoFiles(Array.from(event.target.files || [])));
  $("#camera-input").addEventListener("change", (event) => handlePhotoFiles(Array.from(event.target.files || [])));
